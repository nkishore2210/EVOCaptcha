...
def generate_gif_with_coordinates(canvas_size=512, dot_radius=30, num_frames=50):
    ...
    # Define breathing effect for stationary dot (shrinks to zero and grows back)
    breathing_cycle = np.linspace(1, 0, num_frames // 2).tolist()  # Shrink to zero
    breathing_cycle += np.linspace(0, 1, num_frames // 2).tolist()  # Grow back to full size
    ...
    for frame_index in range(num_frames):
        ...
        # Draw stationary dot with precomputed breathing scale
        stationary_scale = breathing_cycle[frame_index % len(breathing_cycle)]
        scaled_radius = dot_radius * stationary_scale
        if scaled_radius > 0:  # Only draw if radius is greater than zero
            draw.ellipse(
                (
    ...
    frames[0].save(
        ...
        duration=40,  # Reduced frame duration for smoother playback
        loop=0,
    )
    gif_buffer.seek(0)
